...
class ProfessionalBlueprintAI:
    ...
    def __init__(self, settings: AppSettings):
        ...
        logger.info(f"  🧠 Grid cache: {self.grid_cache_size} documents, {self.max_grid_data_size/1024/1024:.1f}MB max each")
    ...
    def _classify_query(self, prompt: str, current_page: Optional[int], 
                       reference_previous: Optional[List[str]]) -> str:
        ...
        if current_page and any(
            keyword in prompt_lower 
            for keyword in ['this page', 'on page', f'page {current_page}', 'here', 'this drawing']
        ):
        ...
        if any(
            keyword in prompt_lower 
            for keyword in ['all', 'every', 'total', 'count', 'list', 'find all', 'show all', 'where are all']
        ):
        ...
        if any(
            keyword in prompt_lower 
            for keyword in ['square footage', 'project name', 'specifications', 'general notes', 'title', 'summary']
        ):
            return 'context_only'
        ...
        if any(
            keyword in prompt_lower 
            for keyword in ['show', 'highlight', 'where', 'locate', 'find', 'identify', 'mark', 'point out']
        ):
    ...
    def _identify_relevant_pages(self, prompt: str, document_text: str, 
                                metadata: Optional[Dict]) -> List[int]:
        ...
        for element_type, pattern in self.element_patterns.items():
            if element_type in prompt_lower or re.search(pattern, prompt, re.IGNORECASE):
        ...
        if not relevant_pages and metadata and 'page_details' in metadata:
            for page_detail in metadata['page_details']:
                if any(term in str(page_detail.get('key_elements', [])).lower() 
...
Remember to think holistically about the building systems and provide insights that add real value beyond just locating elements."""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"OpenAI call failed (attempt {attempt + 1}/{max_retries}): {e}")
                    logger.info(f"Retrying in {delay} seconds...")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"OpenAI call failed after {max_retries} attempts: {e}")
                    raise RuntimeError(f"Failed to get AI response after {max_retries} attempts: {str(e)}")
    ...
    async def _ensure_grid_systems_loaded(
        ...
        if total_cache_size > self.grid_cache_size * self.max_grid_data_size:
            ...
            while (len(self.grid_cache) >= self.grid_cache_size or 
                   total_cache_size > self.grid_cache_size * self.max_grid_data_size):
                if self.grid_cache:
        ...
        try:
            ...
            if data_size > self.max_grid_data_size:
                logger.warning(f"Grid data for {document_id} too large ({data_size} bytes), not caching")
                return
            ...
            logger.info(f"Loaded grid systems for {len(grid_systems)} pages ({data_size} bytes)")
