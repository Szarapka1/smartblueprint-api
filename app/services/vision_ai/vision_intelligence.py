...
Look carefully at all {len(images)} pages provided.
- Count every single {element_type} you can see
- Include {element_type}s shown in plans, elevations, sections, and details
- If you see element tags (like {element_type[0].upper()}1, {element_type[0].upper()}2), count each instance
- Count {element_type}s even if they're shown dashed (existing) or in different line weights

IMPORTANT: You're looking at professional construction drawings. {element_type.capitalize()}s will be shown using standard architectural/engineering symbols.
...
ELEMENT_TAGS: [list any {element_type} tags you see like {element_type[0].upper()}1, {element_type[0].upper()}2, etc.]"""
        ...
        async with self.vision_semaphore:
            response = await self._make_vision_request(
                content,
                system_prompt=f"You are counting {element_type}s in construction drawings. Be thorough and accurate. Construction drawings use standard symbols.",
                max_tokens=1000
            )
        ...
        return {"count": 0, "confidence": "LOW", "pages": [], "tags": []}
...
- Horizontal lines are usually letters (A, B, C, etc.)
- Vertical lines are usually numbers (1, 2, 3, etc.)
- Grid references combine these (like A-1, B-3, C-2)

For EACH {element_type}:
- Give its grid reference location (or describe location if no grid)
- Note any element tag or label
...
TOTAL COUNT: [number]
LOCATIONS:
1. Grid [location] - [tag if any] - [brief description]
2. Grid [location] - [tag if any] - [brief description]
...
(List ALL {element_type}s with their locations)
...
VERIFICATION: Does this count match what you'd expect for this type of drawing?"""
        ...
        async with self.vision_semaphore:
            response = await self._make_vision_request(
                content,
                system_prompt=f"You are verifying {element_type} locations in construction drawings. Be precise with grid references or location descriptions.",
                max_tokens=3000
            )
...
1. SCHEDULES - Look for:
   - {element_type.upper()} SCHEDULE (usually a table)
   - Equipment schedules that might list {element_type}s
   - Any tables showing quantities

2. GENERAL NOTES - Check for:
   - Notes mentioning {element_type}s
   - Installation requirements
   - Quantity statements

3. DRAWING LABELS & TAGS - Find:
   - {element_type} labels (like {element_type[0].upper()}1, {element_type[0].upper()}2)
   - Count how many times each tag appears
   
4. SPECIFICATIONS ON SHEETS - Look for:
...
PROVIDE:
TEXT-BASED COUNT: [number if found in text/schedules]
...
- [any important text about {element_type}s]"""
        ...
        async with self.vision_semaphore:
            response = await self._make_vision_request(
                content,
                system_prompt=f"You are reviewing all text and documentation about {element_type}s. Look everywhere - schedules, notes, specifications, labels.",
                max_tokens=2000
            )
    ...
    def _build_consensus_result(
        ...
        if pass1.get("tags"):
            visual_evidence.append(f"Element tags found: {', '.join(pass1['tags'][:5])}")
    ...
    def _parse_pass1_response(self, response: str, element_type: str) -> Dict[str, Any]:
        """Parse Pass 1 response - enhanced to capture tags"""
        result = {
            "count": 0,
            "confidence": "MEDIUM",
            "pages": [],
    ...
    def _parse_pass2_response(self, response: str, element_type: str) -> Dict[str, Any]:
        ...
        # Extract locations - more flexible pattern
        location_patterns = [
            r'(\d+)\.\s*Grid\s*([A-Z]-?\d+)\s*[-–]\s*(?:\[?([^\]]+?)\]?\s*[-–]\s*)?(.+?)(?=\n\d+\.|$)',
            r'(\d+)\.\s*([A-Z]-?\d+)\s*[-–]\s*(.+?)(?=\n\d+\.|$)',
            r'(\d+)\.\s*(.+?)(?=\n\d+\.|$)'  # Fallback for no grid
        ]
        ...
        for pattern in location_patterns:
            ...
            if matches:
                for match in matches:
                    if len(match.groups()) >= 4:  # Full pattern with grid and tag
                        grid_ref = match.group(2)
                        tag = match.group(3) if match.group(3) else None
                        description = match.group(4).strip()
                    elif len(match.groups()) >= 3:  # Grid without tag
                        grid_ref = match.group(2)
                        tag = None
                        description = match.group(3).strip()
                    else:  # Just description
                        grid_ref = f"Location {match.group(1)}"
                        tag = None
                    ...
                    if grid_ref and not grid_ref.startswith("Location"):
                        result["grid_references"].append(grid_ref)
                
                break  # Use first pattern that works
        
        # If no structured format but we have a count, still record it
        if result["count"] > 0 and not result["locations"]:
            # Create basic locations
            for i in range(min(result["count"], 10)):  # Limit to 10 for performance
                result["locations"].append({
                    "grid_ref": f"See drawing",
                    "visual_details": f"{element_type} #{i+1}",
                    "element_tag": None
                })
    ...
    def _parse_pass3_response(self, response: str, element_type: str) -> Dict[str, Any]:
        ...
        # Extract schedule count - handle both number and "No schedule found"
        schedule_match = re.search(r'SCHEDULE_COUNT:\s*(.+?)(?:\n|$)', response, re.IGNORECASE)
        if schedule_match:
            schedule_text = schedule_match.group(1).strip()
            if "no schedule" in schedule_text.lower():
                result["schedule_count"] = None
            else:
                # Try to extract number
                num_match = re.search(r'(\d+)', schedule_text)
                if num_match:
                    result["schedule_count"] = int(num_match.group(1))
